...
import { isClosure, makeClosure, Closure, Value, makeDictVal, DictVal, isDictVal, isSymbolSExp } from "./L32-value";
...
const evalDict = (exp: DictExp, env: Env): Result<Value> =>
    bind(mapResult((b) => L32applicativeEval(b.val, env), exp.entries), (values: Value[]) => {
        const keys = exp.entries.map((b) => b.var.var); 
        const entries: [string, Value][] = keys.map((k, i) => [k, values[i]]);
...
const L32applyProcedure = (proc: Value, args: Value[], env: Env): Result<Value> =>
    ...
    isDictVal(proc) ? applyDict(proc, args) :
...
const applyDict = (dict: DictVal, args: Value[]): Result<Value> => {
    if (args.length !== 1) {
        return makeFailure(`Dict expects exactly one argument`);
    }
    const keyArg = args[0];
    const key = isSymbolSExp(keyArg) ? keyArg.val : isString(keyArg) ? keyArg : null;
    if (key === null) {
        return makeFailure(`Dict key must be a string or symbol`);
    }
    const match = dict.entries.find(([k, _v]) => k === key);
    return match ? makeOk(match[1]) : makeFailure(`Key ${key} not found in dict`);
};
