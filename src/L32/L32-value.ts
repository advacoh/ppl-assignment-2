...
export type SExpValue = number | boolean | string | PrimOp | Closure | SymbolSExp | EmptySExp | CompoundSExp | DictVal;
...
export const makeDictVal = (entries: [string, Value][]): DictVal => ({ tag: "DictVal", entries });


export const isDictVal = (x: any): x is DictVal => x.tag === "DictVal";
...
export const valueToString = (val: Value): string =>
    ...
    isDictVal(val) ? `dict(${val.entries.map(([k, v]) => `${k} . ${valueToString(v)}`).join(' ')})` :
    val;
