...
export type DictExp = {tag: "DictExp"; entries: Binding[]; }
...
export const parseDictExp = (bindingsSexp: Sexp[]): Result<DictExp> => {
    if (!isGoodBindings(bindingsSexp)) {
        return makeFailure('Malformed bindings in "dict" expression');
    }
    const vars = map(b => b[0], bindingsSexp);             // ["a","b"]
    const valsResult = mapResult(parseL32CExp, map(second, bindingsSexp));
    return bind(valsResult, (vals: CExp[]) =>
        makeOk(makeDictExp(zipWith(makeBinding, vars, vals))));
};
...
const unparseDictExp = (de: DictExp): string => 
    `(dict ${map((b: Binding) => `(${b.var.var} ${unparseL32(b.val)})`, de.entries).join(" ")})` 
