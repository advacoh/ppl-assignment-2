...
export const makeDictVal = (entries: [string, Value][]): DictVal =>
    ({tag: "DictVal", entries: entries});

export const isDict = (x: any): x is DictVal => x.tag === "DictVal";
...
export const valueToString = (val: Value): string =>
    ...
    isDict(val) ? `{${val.entries.map(([k, v]) => `${k}: ${valueToString(v)}`).join(', ')}}` :
    val
